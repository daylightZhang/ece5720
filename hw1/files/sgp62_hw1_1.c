...
for(int n = MIN_SIZE; n <= MAX_SIZE; n = 2*n){ //NOTE: what is for array A[0:n-1]? For each of the arrays? Each array is A at a different n, as n goes from MIN_SIZE to MAX_SIZE
  //Double the stride
  for(int s = 1; s <= n/2; s += s){
    //start the timer
    int Tstart = clock_gettime(CLOCK_MONOTONIC, &start);
    ...
    //Create subarray and store in it
  }
}
// Loop over strides
